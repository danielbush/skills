...
4. **Project collaboration framework** — standard artifacts (AGENTS.md, SESSIONS.md, work/, docs/) that bridge sessions and give structure
...
1. **Continue where we left off?** — read the last entry in [SESSIONS.md](SESSIONS.md) and scan `work/active/` to summarise current state and ask which thread to pick up
...
When continuing, always read `work/active/` to orient.
